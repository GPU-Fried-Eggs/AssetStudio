...
partial class AssetStudioGUIForm : Form
{
    ...
    private void PreviewAudioClip(AssetItem assetItem, AudioClip m_AudioClip)
    {
        ...
        var result = system.createSound(m_AudioData, MODE.OPENMEMORY | loopMode, ref exinfo, out sound);
        ...
        result = sound.getLength(out FMODlenms, TIMEUNIT.MS);
        if (ERRCHECK(result)) return;
    ...
    private void FMODprogressBar_MouseUp(object sender, MouseEventArgs e)
    {
        if (channel != null)
        {
            ...
            var result = channel.setPosition(newms, TIMEUNIT.MS);
            if ((result != RESULT.OK) && (result != RESULT.ERR_INVALID_HANDLE))
            {
                if (ERRCHECK(result)) { return; }
            }
    ...
    private void timer_Tick(object sender, EventArgs e)
    {
        ...
        if (channel != null)
        {
            var result = channel.getPosition(out ms, TIMEUNIT.MS);
            if ((result != RESULT.OK) && (result != RESULT.ERR_INVALID_HANDLE))
            {
                ERRCHECK(result);
            }
